...
export class CompactTablePresentationService extends TablePresentationService {
  ...
  globalExclusions: ReadonlyArray<string> = [
    '@class', 'name', 'uid',
  ...
  sanitizeResultSet(resultSet: AqlResultMetadata): SanitizedResultSet {
    ...
    if (tables) {
      tables.forEach((dataSet, tableName) => {
        this.compactViewPresentationData(resultSet.executedAql, dataSet, tableName, Object.keys(dataSet));
      });
    }
  ...
  private compactViewPresentationData(aql: string, results: any, tableName: string, tableNames: string[]): void {
    this.parseResultValues(aql, results, tableName, tableNames, this.includePresentation);
  ...
  private prepareCompactHeaders(data: TableNode): TableNode {
    const compactTables = [];
  ...
  private includePresentation(key: string, result: any): boolean {
