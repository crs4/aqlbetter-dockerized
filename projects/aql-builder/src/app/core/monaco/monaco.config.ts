...
export const language = {
  ...
  operators: [
    // Logical
    'ALL', 'AND', 'ANY', 'BETWEEN', 'EXISTS', 'IN', 'LIKE', 'NOT', 'OR', 'SOME', 'MATCHES',
    // Set
  ...
  builtinFunctions: [
    ...
    // Custom
    'SQUASH', 'TAGS', 'CONCAT', 'CURRENT_TIMESTAMP'
  ],
  builtinVariables: [
    // Configuration
  ...
  tokenizer: {
    root: [
      ...
      { include: '@strings' },
      { include: '@scopes' },
      { include: '@aqlIdentifiers' },
      { include: '@aqlVariables' },
      [/[;,.]/, 'delimiter'],
      [/[()]/, '@brackets'],
      [/[\w@#$]+/, {
        cases: {
    ...
    aqlIdentifiers: [
      ['EHR', {token: 'aql.identifier.ehr'}],
      ['COMPOSITION', {token: 'aql.identifier.composition'}],
      ['EVALUATION', {token: 'aql.identifier.evaluation'}],
      ['OBSERVATION', {token: 'aql.identifier.observation'}],
      ['INSTRUCTION', {token: 'aql.identifier.instruction'}],
      ['ACTION', {token: 'aql.identifier.action'}],
      ['SECTION', {token: 'aql.identifier.section'}],
      ['VERSIONED_OBJECT', {token: 'aql.identifier.versioned_object'}],
      ['VERSION', {token: 'aql.identifier.version'}],
      ['ADMIN_ENTRY', {token: 'aql.identifier.admin_entry'}],
      ['CLUSTER', {token: 'aql.identifier.cluster'}],
      ['TASK_PLAN', {token: 'aql.identifier.task_plan'}],
      ['WORK_PLAN', {token: 'aql.identifier.work_plan'}]
    ],
    aqlVariables: [
      [/\w*\/(\w)+/, 'aql.path'],
      [/\w*#\w+(\/\w*)?/, 'aql.path.short'],
      [/:\w*/, 'aql.path.variable']
    ],
  }
} as ILanguage;
